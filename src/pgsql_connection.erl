...
-import(utf8, [to_binary/1,from_binary/1]).
...
get_parameter(C, Name) ->
    gen_fsm:sync_send_event(C, {get_parameter, el_to_binary(Name)}).
...
el_to_binary(B) when is_binary(B) -> B;
el_to_binary(L) when is_list(L)   -> utf8:to_binary(L).
